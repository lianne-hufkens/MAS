...
def plotBar():
    ...
    if not PureCommit:
        ...
        p1 = plt.bar(x, y1, color=buyerPalette[0], edgecolor='white', width=barWidth)
        p2 = plt.bar(x, y2, bottom=y1, color=buyerPalette[1], edgecolor='white', width=barWidth)
        plt.xticks(x, xTickLabelsBuyer, fontsize=6, rotation=30)
        plt.legend((p1[0], p2[0]), ('Buy', 'Returned'))
        plt.subplot(2, 1, 2)

        plt.title("Seller (Total Returned Product)")
        y2 = np.array([s.auctionCancel for s in sellers])
        y1 = np.array([s.auctionSell for s in sellers])
        x = range(len(sellers))
        plt.ylabel('Total returned Item')
        p1=plt.bar(x, y1, color=sellerPalette[0], edgecolor='white', width=barWidth)
        p2 = plt.bar(x, y2,bottom=y1, color=sellerPalette[1], edgecolor='white', width=barWidth)
        plt.xticks(x, xTickLabelsSeller, fontsize=6, rotation=30)
        plt.legend((p1[0], p2[0]), ('Sell', 'Returned'))
    else:
        plt.subplot(2, 1, 1)
        barWidth = 0.5
        ...
        p1 = plt.bar(x, y, color=buyerPalette, edgecolor='white', width=barWidth)
        plt.xticks(x, xTickLabelsBuyer, fontsize=6, rotation=30)
        plt.subplot(2, 1, 2)
        y = np.array([s.auctionSell for s in sellers])
        plt.title("Seller (Total Item)")
        x = range(len(sellers))
        plt.ylabel('Total Item')
        p1 = plt.bar(x, y, color=sellerPalette, edgecolor='white', width=barWidth)
        plt.xticks(x, xTickLabelsSeller, fontsize=6, rotation=30)
